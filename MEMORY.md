...
- **🔴 5 YILA İNDİRME (prune) — 1 TEMMUZ:** `venv/Scripts/python.exe scripts/prune_old_vectors.py --before 2021-06-28 --apply` (default ns: ~5.910 rapor / ~7.197 chunk siler). Yazma kotası 1 Temmuz'a kadar dolu → silme bloklu; o tarihte çalıştır → panelde "before 2021" satırı kaybolur, vektör ~31.6K→~24.4K. `INGEST_LOOKBACK_YEARS=5` zaten ayarlı; yalnız aktif `default` ns (v2/pilot'a DOKUNMA).
