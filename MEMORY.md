...
1. **🔴 1 TEMMUZ VERİ AKTİVASYONU (EN ÖNCELİKLİ, artık YAPILABİLİR).** Pinecone yazma kotası "1 Temmuz'a kadar bloklu"ydu → o pencere GEÇTİ, kota yenilenmiş olmalı. Önce **Faz 0 kota-kapısı** (dummy upsert → 429-yok kontrolü); geçerse temiz `v3` namespace'e **tüm-ülke × son-1-yıl × ülke-başı-cap** backfill → cutover (`PINECONE_NAMESPACE=v3`) → retention + Cloudflare cron AÇ (`RETENTION_DAYS`/`RETENTION_PER_COUNTRY_CAP`/`INGEST_TRIGGER_TOKEN` Render env) → eski `default`/`v2`/`pilot` sil. Kod master'da PASİF/hazır; yalnız çalıştırma. Tam prosedür (Faz 0-5): `~/.claude/plans/ilk-olarak-chat-b-l-m-nde-mighty-milner.md`.
