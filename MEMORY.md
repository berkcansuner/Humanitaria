...
2. **M&E Reports sonraki fazları (Faz A ✅ 2026-07-11'de bitti — rapor türleri):** **Faz B** = AI görseller (kapak + bölüm illüstrasyonları, Gemini image API, base64 data-URI gömme — `report_pdf.py` SSRF koruması yalnız `data:` geçirir) / **Faz C** = rakam structured-extraction + kodla çizilen grafikler (PDF'e PNG çünkü xhtml2pdf inline SVG desteklemez; web'e SVG) + iki-dönem karşılaştırmalı/trend raporu / **Faz D** = donör/kurumsal format (kullanıcıdan şablon girdisi gerekir). Her biri kendi brainstorm→spec→plan döngüsü.
