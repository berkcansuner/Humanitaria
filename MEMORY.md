...
**Son güncelleme:** 2026-06-29 (M&E Raporları özelliği + rapor kalite cilası CANLI; **SIRADAKİ = 1 TEMMUZ VERİ AKTİVASYONU** — v3 backfill + kayan-pencere retention, plan dosyasında hazır)

---

## 📍 NEREDE KALMIŞTIK (10 saniyelik özet)

**▶▶ SONRAKİ SEANS (1 TEMMUZ) — TEK ANA İŞ: VERİ AKTİVASYONU.** Pinecone yazma kotası 1 Tem'de yenilenir (dashboard: "Writes paused until July 1, 2026") → temiz **`v3`** namespace'e **tüm-ülke × son-1-yıl × ülke-başı-cap** backfill → cutover → rolling-window retention + Cloudflare Worker cron AÇ → eski `default`/`v2`/`pilot` sil. **Kod yazılı + master'da PASİF; yalnız ÇALIŞTIRMA/AKTİVASYON kaldı.** Tam prosedür (Faz 0-5): plan dosyası `~/.claude/plans/ilk-olarak-chat-b-l-m-nde-mighty-milner.md` + aşağıdaki "SONRAKİ SEANS — Rollout" bölümü. Önce Faz 0 kota-kapısı (dummy upsert 429-yok kontrolü). **Ayrıca bekleyen kullanıcı işi:** sızan Render API key + Google secret rotasyonu (aşağıda #1).
