...
1. **✅ Faz 5 de TAMAM (2026-07-10, kullanıcı onayıyla):** eski `default` namespace silindi → index'te YALNIZ `v3` (25.668 vektör) var. Veri aktivasyonu projesi KAPANDI.
2. **🔴 KULLANICI İŞİ:** sızan Render API key + Google client secret rotasyonu (hâlâ açık).
3. **Eval setini v3 korpusundan yeniden üret** (`scripts/build_eval_labels.py`) — mevcut etiketler v3'e karşı geçersiz (yalnız 14/136 doküman v3'te).
4. **TIER 3 perf (eval'li)** — rewrite+extract birleştirme / Gemini context caching fizibilite / hybrid arama. **TIER 4** — SQLite→Postgres, frontend lint. Backlog: `~/.claude/plans/nerede-kalm-t-k-sequential-alpaca.md`.
