...
1. **▶ BURADAN DEVAM ET (karar verildi 2026-07-10, 4. iş — kullanıcı "kaldığımız yerden devam" dedi):** admin kullanıcı yönetimi + ayarlar/profil sayfası + arayüz dil seçici (TR/EN) — kullanıcı bu üçünü ayrı ayrı değil TEK turda brainstorm→plan etmek istedi. Bu oturumun keşfinden ön-bilgi (yeniden keşfetmeye gerek yok): (a) **admin kullanıcı yönetimi** — mevcut `ADMIN_EMAILS` allowlist deseni (`api/routes/admin.py`, `get_admin_user`) genişletilebilir bir temel; (b) **ayarlar/profil sayfası** — şu an yalnız tema toggle'ı var (header'da, `ThemeToggle.vue`), sayfanın içeriği (şifre değiştirme? hesap silme? varsayılan ülke/tema tercihi?) ÖNCE netleştirilmeli, kapsamı belirsiz; (c) **arayüz dil seçici** — EN BÜYÜK/EN RİSKLİ parça: Landing/Auth/Chat/Reports/Admin'in hepsinde sabit İngilizce metin var (i18n kütüphanesi yok), tam bir tarama+entegrasyon gerektirir — muhtemelen (a)+(b)'den ayrı, kendi planına bölünmeli.
