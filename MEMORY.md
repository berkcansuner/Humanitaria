...
**Son güncelleme:** 2026-06-28 (veri stratejisi: tüm-ülke 1-yıl + ülke-başı cap + günlük kayan-pencere otomasyonu — kod yazıldı, PASİF, 1 Temmuz'da açılır)
...
- **▶ SONRAKİ SEANS — özellik yol haritası (admin paneli BİTTİ + prod'da canlı; kullanıcı sıradakini sonraki seansa bıraktı):** 3 aday — bu seansta Explore×3 ile keşfedildi, **ZATEN VAR olanları tekrar yapma** (regenerate/edit/konuşma-arama/tıklanabilir atıflar mevcut; tek-namespace breakdown var). **(a) M&E analiz** — çoklu-rapor özetleme + gösterge/sayı çıkarma (structured extraction) + ülke-zaman karşılaştırma; LLM kurulumu `rag/chain.py`'dan reuse, yeni `rag/summarizer.py`/`extraction.py`. **(b) RAG kalite cilası** — akıllı "belge bulunamadı" önerileri (`chat.py._no_docs_message`), "güncel/latest" tarih davranışı + recency hedefleme (`query_processor.should_boost_recency`). **(c) Sohbet export/paylaş** — PDF/MD indirme + salt-okunur link (yeni `utils/exportChat.js` + backend share endpoint). NOT: hybrid arama yeni Pinecone index ister (pahalı, ayrı iş); frontend'de grafik kütüphanesi YOK (CSS bar/tablo deseni). Breakdown taraması Path-B optimizasyonu (`query`+`include_values=False`, ~30-60s) opsiyonel.
