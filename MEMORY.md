...
1. **Kullanıcıya görünür adaylar (2026-07-10'da sunuldu, seçilmedi):** (a) **sohbet export/paylaş** — PDF/MD indirme + salt-okunur link (rapor xhtml2pdf altyapısı yeniden kullanılır; kullanıcıya önerilen ilk seçenekti) / (b) **RAG kalite cilası** — akıllı "belge bulunamadı" önerileri + "güncel/latest" tazelik hedeflemesi.
2. **Teknik backlog:** TIER 3.2 Gemini context caching fizibilite (OpenAI-uyumlu endpoint destekliyor mu araştır) / 3.4 hybrid-sparse arama (yeni Pinecone index, pahalı, ayrı iş) / 4.7 kod tekrarı temizliği. Backlog: `~/.claude/plans/nerede-kalm-t-k-sequential-alpaca.md`.
3. ✅ TAMAMLANANLAR (2026-07-10, tek gün): v3 veri aktivasyonu (Faz 0-5, cutover+cron canlı) · eval seti v3'ten yeniden üretildi · Supabase Postgres göçü (kalıcılık KAPANDI) · **TIER 3.1 birleşik query planner (takip TTFT ~3s azaldı)** · **TIER 4.2 frontend lint+prettier+CI** — detay üstteki bloklarda.
2. **Ertelenen eval-gerektiren TIER 3 perf** (kullanıcı bu seans erteledi): 3.1 rewrite+extract birleştirme [en büyük TTFT kazanımı, eval-hassas → `scripts/eval_rag.py --metrics` regresyon-yok şart] / 3.2 Gemini context caching [önce fizibilite araştırması: OpenAI-uyumlu endpoint destekliyor mu] / 3.4 hybrid-sparse arama [pahalı, yeni Pinecone index, ayrı iş].
3. **TIER 4** (kalıcılık/frontend): 4.1 SQLite→Postgres göçü (free-tier restart'ta kullanıcı+sohbet kaybı) / 4.2 frontend lint+prettier / 4.7 kod tekrarı temizliği. Backlog: `~/.claude/plans/nerede-kalm-t-k-sequential-alpaca.md`.
4. **🔴 KULLANICININ MANUEL İŞİ (kod değil):** sızan Render API key + Google client secret rotasyonu (aşağıda #1).
