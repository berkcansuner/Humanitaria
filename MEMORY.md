...
1. **🔴 KULLANICI İŞİ:** sızan Render API key + Google client secret rotasyonu (hâlâ açık).
2. **Opsiyonel backlog:** TIER 3.2 Gemini context caching fizibilite (OpenAI-uyumlu endpoint destekliyor mu araştır) / 3.4 hybrid-sparse arama (yeni Pinecone index, pahalı, ayrı iş) / 4.7 kod tekrarı temizliği. Backlog: `~/.claude/plans/nerede-kalm-t-k-sequential-alpaca.md`.
