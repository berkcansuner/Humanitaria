...
2. **TIER 3 perf (eval kapısı HAZIR: recall@5=0.253/MRR=0.528/nDCG=0.327)** — 3.1 rewrite+extract tek-çağrı birleştirme (en büyük TTFT kazanımı, eval regresyon-yok şartı) / 3.2 Gemini context caching fizibilite / 3.4 hybrid arama (pahalı, ayrı iş).
3. **TIER 4 kalanlar** — 4.2 frontend lint+prettier, 4.7 kod tekrarı temizliği. Backlog: `~/.claude/plans/nerede-kalm-t-k-sequential-alpaca.md`.
4. ✅ TAMAMLANANLAR (2026-07-10): v3 veri aktivasyonu (Faz 0-5, cutover+cron canlı) · eval seti v3'ten yeniden üretildi · **Supabase Postgres göçü (kalıcılık sorunu KAPANDI)** — detay üstteki iki blokta.
