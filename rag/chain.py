...
_INDICATOR_SYSTEM_PROMPT = (
    ...
    "columns — 'Indicator', 'Latest value', 'As of', 'Source' (translate these column labels into the "
    "requested language; leave only the [n] citation-marker format unchanged). One row per indicator that is ACTUALLY "
    ...
    "is the date the source states the value was recorded — not today's date. The 'Source' column holds "
    "ONLY the citation marker, e.g. '[2]'. Do NOT invent an indicator that has no reported value in the "
