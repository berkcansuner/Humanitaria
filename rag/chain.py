...
_REPORT_SYSTEM_PROMPT = (
    ...
    "DISTIL, do NOT duplicate: state each headline figure once here at a high level; do NOT also reproduce "
    "the same numbers and their full breakdown verbatim in the body. The granular detail (e.g. the per-Phase "
    "IPC breakdown, sub-totals, beneficiary counts) belongs in the body, not repeated in the Summary.\n"
    ...
    "overview, WFP/FAO/UNICEF for their own figures); when several are equivalent, the most recent. NEVER cite "
    "the same single fact from more than one source. Use several markers [n][m] ONLY when a sentence genuinely "
    "combines DISTINCT facts from different documents — then cite each document actually used. Cite ONLY numbers "
    "that appear as [n] in the Context; never invent one.\n"
